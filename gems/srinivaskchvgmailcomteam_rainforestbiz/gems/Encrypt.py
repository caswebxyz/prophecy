...
class Encrypt(ComponentSpec):
    name: str = "Encrypt"
    category: str = "Transform"
    gemDescription: str = "Edits column names or values using expressions."
    docUrl: str = "https://docs.prophecy.io/low-code-spark/gems/transform/Encrypt/"
    ...
    class EncryptProperties(ComponentProperties):
    ...
    def onClickFunc(self, portId: str, column: str, state: Component[EncryptProperties]):
    ...
    def allColumnsSelectionFunc(self, portId: str, state: Component[EncryptProperties]):
    ...
    def expressions_to_csv(self, state: Component[EncryptProperties]) -> Component[EncryptProperties]:
        exprCSV = []
    ...
    def csv_to_expressions(self, state: Component[EncryptProperties]) -> Component[EncryptProperties]:
        exprs = []
    ...
    def dialog(self) -> Dialog:
        expTable = ExpTable("Encrypt Expression") \
            .enableVirtualization() \
            .bindProperty("expressions") \
            .withCopilotEnabledExpressions().allowCopilotExpressionsFix()
        bulkEdit = StackLayout(height="100%") \
            .addElement(
            NativeText("Edit the Encrypt expressions in the field below. Use the format of \"name,expr\".")
        ).addElement(
        ...
        return Dialog("Encrypt").addElement(
    ...
    def validate(self, context: WorkflowContext, component: Component[EncryptProperties]) -> List[Diagnostic]:
        diagnostics = []
    ...
    def onChange(self, context: WorkflowContext, oldState: Component[EncryptProperties], newState: Component[EncryptProperties]) -> Component[
        EncryptProperties]:
    ...
    def getCPStmt(self, component: Component[EncryptProperties]) -> Optional[SelectStmt]:
        return SelectStmt(
            component.component,
    ...
    class EncryptCode(ComponentCode):
        def __init__(self, newProps):
            self.props: Encrypt.EncryptProperties = newProps
